...
- 災害速報タブの3番目は、熱中症警戒情報の運用期間（4/22〜10/21）は「熱中症」、それ以外（10/22〜4/21）は「積雪」に入れ替わる（`AmedasSnow.isSeason` ＝ `!HeatAlerts.isInSeason`。判定は JST の月日）。両方が同時に出ることはない。`_loadHeat` が運用期間外なら `_loadSnow` を呼ぶ
- 積雪は気象庁アメダスの10分値 `bosai/amedas/data/map/<yyyyMMddHHmm00>.json`（最新時刻は `latest_time.txt`。`+09:00` 付きなので `toJstWallClock` で壁時計に戻してファイル名を組む）。**積雪のある地点にだけ `snow`（積雪深cm）・`snow1h/6h/12h/24h`（降雪量cm）のキーが現れ、無雪期はキー自体が無い**。値は `[値, 品質フラグ]` で欠測は値が null。同じ観測時刻のあいだは再取得しない（`app/lib/data/amedas_snow.dart`）
