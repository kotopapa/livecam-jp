...
- 初回収集: 採用90データセット / 148,871件（消防水利147,885・給水877・備蓄109）/ **22都道府県・78自治体のみ（全市区町村の約4%）**。ライセンスは全件CC BY系で再配布可
...
- 未取得の大口はXLSX配信（東京消防庁・福岡市など13件）。openpyxl を足して `read_xlsx()` を作るのが次の一手
