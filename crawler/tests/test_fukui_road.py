...
def test_image_url_uses_data_image_not_record_id():
    ...
    raw = (Path(__file__).parent / "fixtures" / "fukui_road_cameras.json").read_text(encoding="utf-8") \
        if (Path(__file__).parent / "fixtures" / "fukui_road_cameras.json").exists() else None
    if raw is None:
        import pytest
        pytest.skip("fixture missing")
    cams = {c["id"]: c for c in parse_cameras(raw)}
