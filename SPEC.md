...
1. 官公庁が公開している全国のライブカメラを **クローラで台帳化** し、**死活監視して静的JSONで配信** し、**iOSアプリが地図に表示する**。
  ...
  "cameras": [
    {
      ...
      "lat": 35.5942,
      "lng": 139.6647,
      "coord_accuracy": "exact",              // exact | approx | town_level
...
## 9. Phase 4 — iOSアプリ（`ios/`）

### 9.1 技術スタック

- Swift 6 / SwiftUI / iOS 17.0+
- 地図：**MapKit**（`Map` + `Annotation`）。追加コストなし
- 永続化：SwiftData（お気に入り、キャッシュメタ）
- 画像キャッシュ：自前の軽量実装（URLCache + ディスク）。外部ライブラリは入れない
- YouTube再生：`WKWebView` + IFrame Player
- 非同期：Swift Concurrency（`async/await`, `TaskGroup`）
- 依存パッケージ：**原則ゼロ**
...
- 全国の承認済みカメラをピン表示。カテゴリ別にピン色を変える（河川=青、道路=灰、火山=赤、海岸=水色）
- ズームレベルに応じてクラスタリング（`MKClusterAnnotation`）
- `status.state == "error"` のカメラは**表示しない**
- `frozen` はピンを半透明にする
- 現在地ボタン、カテゴリフィルタ、都道府県ジャンプ
- ピンをタップ → 下部にプレビューカード（サムネイル + 名前 + 取得時刻 + 出典）
- カードをタップ → 詳細画面

**② 詳細画面**

- 画像/映像の表示（`feed.type` による分岐）
- **取得時刻を必ず大きく表示**（「3分前」ではなく「06:12 取得」と絶対時刻も出す）
- 手動更新ボタン（**最短60秒のクールダウンを強制**。連打させない）
- 出典表示（運営者名 + 元ページへのリンク）
- お気に入り追加
...
- グリッド表示。各カードにサムネイルと取得時刻
- 一括更新（順次取得。同時実行は3まで）

**④ 検索画面**

- カメラ名・河川名・路線名・自治体名でインクリメンタルサーチ
- 現在地から近い順のリスト

**⑤ ウィジェット（WidgetKit）**

- お気に入り1台の最新画像 + 取得時刻
- 更新は `TimelineProvider` で15分間隔（システム都合で伸びることを前提に、画像に取得時刻を焼き込む）

### 9.3 オンデバイス画像判定

サーバーを使わず端末で完結させる。**外部モデルは不要**。以下は画像統計量だけで実装できる：
...
Vision フレームワークの `VNGenerateImageFeaturePrintRequest` は前回画像との差分検出に使える（端末側でのフリーズ検知）。

**「今見る価値があるカメラ」ランキング**は、お気に入り + 現在地周辺のカメラに対してのみ計算する（全国分を端末で回さない）。

### 9.4 アクセス制御（重要）

一次ソースへの負荷を抑えるため、アプリ側で以下を**強制**する：

- 同一カメラの再取得は最短60秒（`status.avg_interval_sec` が判明していればそれに従う）
- 画面に見えていないカメラは取得しない
- バックグラウンドでの定期取得はしない（v1）
- 同時接続は3まで
- `If-None-Match` を送り、304 なら再ダウンロードしない

### 9.5 表示上の必須要素

- **免責**：初回起動時と詳細画面のフッターに以下の趣旨を表示する。
...
| 個人・企業設置のカメラを見つけた | v1スコープ外。`rejected` にして理由を記録 |
...
| カメラの位置が誤っている | 誤った避難判断 | 座標未確定は掲載しない。`coord_accuracy` をアプリに表示 |
